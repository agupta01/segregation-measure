...
def save_results(city_name, step_count, chunk_entropies=None, random_entropies=None):
    ...
    plt.plot(
        np.repeat(
            chain_to_entropy(dict(init_partition.assignment), race_matrix), step_count
        ),
        c="r",
    )

    plt.suptitle(
        f"Chunk Flip Entropies for {city_name}, burn-in for {(0.1)*step_count} steps",
        y=1,
    )
    plt.tight_layout()
    ...
    plt.suptitle(
        f"Random Flip Entropies for {city_name}, burn-in for {(0.1)*step_count} steps",
        y=1,
    )
    plt.tight_layout()
...
def generate_entropies(_type, step_count, burn_in, results, processes=5):
    ...
    g = Process(
        target=generator, args=(stop_event, partition_queue, _type, step_count, burn_in)
    )
    workers = [
